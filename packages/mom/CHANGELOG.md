...
- Changed mom to read workspace settings from `.pi/settings.json`, ignore workspace extension authorities in strict mode, post channel mention replies in the triggering Slack thread, persist separate session context per channel mention thread, queue same-channel Slack requests instead of rejecting them with a shared per-channel pending cap, and keep execution serialized per channel while DMs remain channel-scoped
